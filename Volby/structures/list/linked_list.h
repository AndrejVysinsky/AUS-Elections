...
namespace structures
{
	...
	inline LinkedList<T>& LinkedList<T>::operator=(const LinkedList<T>& other)
	{
		...
		{
			clear();
			LinkedListItem<T>* hlava = other.first_;
			for (size_t i = 0; i < other.size_; i++)
			{
				add(hlava->accessData());
				hlava = hlava->getNext();
			}
	...
	inline T LinkedList<T>::removeAt(const int index)
	{
		...
		if (size_ == 1) {
			...
			if (index == 0) {
				...
				LinkedListItem<T>* predosly = getItemAtIndex(index - 1);
				vymazany = predosly->getNext();
				predosly->setNext(vymazany->getNext());
				if (index == size_ - 1)
					last_ = predosly;
			}
		}

		T vysledok = vymazany->accessData();
		delete vymazany;
		size_--;
		return vysledok;
	}
